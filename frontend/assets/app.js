...
const renderAdminView = async () => {
    ...
    switch (view) {
        case "add":
            const [manageArtist, managePO, mangeAlbum] = elements([
                "a",
                "a",
                "a",
            ]);
            manageArtist.innerText = "Add an artist";
            manageArtist.setAttribute(
                "href",
                "/admin/manage-artist?action=new",
            );
        ...
        case "artists":
            {
                const [nameOption, modifiedOption] = elements([
                    "option",
                    "option",
                ]);
                ...
                const { artists, pages } = await fetch(apiUrl).then(
                    (response) => response.json(),
                );

                const [table, tableBody, header] = elements([
                    "table",
                    "tbody",
                    "tr",
                ]);
        ...
        case "purchase-orders":
            {
                ...
                const { purchase_orders } = await fetch(apiUrl).then(
                    (response) => response.json(),
                );
                ...
                headers.forEach((header) => {
                    ...
                    switch (header) {
                        case "estimated_receipt":
                            newCell.innerHTML = dispatch[header].substring(
                                0,
                                16,
                            );
                            break;
    ...
    for (let index = 1; index < nLines; index++) {
        Array.from(rows[index].children).forEach((cell) => {
            ...
            cell.firstChild.name =
                inputName.substring(0, pos + 1) + String(index);
        });
    }
};
...
const deleteLine = () => {
    ...
    for (let index = 1; index < nLines; index++) {
        const rowAlbumID = document.querySelector(
...
const handleAddDelButtons = () => {
    ...
    for (let index = 1; index < nLines; index++) {
        ...
        const rowStock = document.querySelector(
...
const sendOrder = async (event) => {
    ...
    ["purchase_order", "estimated_delivery", "dispatch_cost"].forEach(
        (field) => {
            currentForm.append(
                field,
...
const renderPurchaseForm = async () => {
    ...
    switch (action) {
        case "view":
            ...
            lines.forEach((poLine) => {
                ...
                document
                    .getElementById("purchase-order-lines")
                    .appendChild(newRow);
            });
            break;
        case "new":
    ...
    if (addable) {
        ...
        const {
            ...
            `/api/client/artists/${artistSelect.value}?view=user`,
        ).then((response) => response.json());
...
const sendArtist = async (event) => {
    ...
    if (
        url.get("action") === "edit" &&
        currentForm.get("artist_id").length > 0
    ) {
        method = "PATCH";
...
const sendAlbum = async (event) => {
    ...
    if (
        url.get("action") === "edit" &&
        currentForm.get("album_id").length > 0
    ) {
        method = "PATCH";
...
const changeView = async (event) => {
    ...
    switch (view) {
        ...
        case "artists":
            urlParams =
                "?view=artists&page=1&sort=modified&direction=descending";
            break;
        case "purchase-orders":
...
const checkTime = (event) => {
    ...
    const validControl =
        ctrlKey && ["a", "p", "c", "x", "z", "v"].includes(key);
...
const auth = async (event) => {
    ...
    const apiUrl =
        pathname === "/register" ? "/auth/register" : "/auth/sign-in";
...
const renderOrders = async () => {
    ...
    if (hasCart) {
        ...
        const [cartHeader, orderBtn, balanceP] = elements([
            "h2",
            "button",
            "p",
        ]);
    ...
    if (orders.length > 0) {
        ...
        orders.forEach((order) => {
            Object.keys(order)
                .filter((item) =>
                    ["order_id", "dispatched", "balance"].includes(item),
                )
                .forEach((key) => {
...
const checkOut = async () => {
    const request = await fetch("/api/client/cart/checkout", {
        method: "POST",
    });
...
const renderAlbum = async () => {
    ...
    if (cart !== undefined) {
        ...
        if (cart > 0) {
            ...
            cartInfo.innerText =
                "no albums in stock. we will replenish this soon.";
        }

...
const checkCatalog = async () => {
  ...
  const artistElement = document.querySelector("[name=artist_id]");
  let artistName = null;

  Array.from(artistElement.options).forEach((option) => {
    if (option.value === artistElement.value) {
      artistName = option.innerHTML;
    }
  });

  const albums = await fetch(`/api/admin/catalog/${artistName}/${title}`).then(
    (response) => response.json()
  );
